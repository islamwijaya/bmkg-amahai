...
<div class="max-w-2xl">
    <div class="bg-white rounded-2xl shadow-sm p-6">
        <h2 class="text-lg font-bold text-gray-800 mb-6">Unggah Dokumen Transparansi Kinerja</h2>
        ...
        <form action="{{ route('admin.transparansi.store') }}" method="POST" enctype="multipart/form-data" class="space-y-5">
            @csrf

            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">Judul Dokumen <span class="text-red-500">*</span></label>
                <input type="text" name="title" value="{{ old('title') }}" required
                       placeholder="cth: Rencana Kinerja Tahunan Stamet Amahai Tahun 2025"
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4" x-data="{ category: '{{ old('category') }}' }">
                <div>
                    ...
                    <div x-show="category === 'custom'" x-cloak x-transition class="mt-3">
                        ...
                        <input type="text" name="custom_category" value="{{ old('custom_category') }}"
            ...
            <div>
                <label class="block text-sm font-semibold text-gray-700 mb-1.5">File PDF <span class="text-red-500">*</span></label>
                <label class="flex flex-col items-center justify-center gap-2 border-2 border-dashed border-gray-200 rounded-xl p-8 cursor-pointer hover:border-bmkg-blue hover:bg-bmkg-sky/30 transition-colors">
                    <svg class="w-10 h-10 text-gray-300" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/>
                    </svg>
                    <span class="text-sm text-gray-500" id="file-label">Klik untuk pilih file PDF</span>
                    <span class="text-xs text-gray-400">Maks. 10 MB</span>
                    <input type="file" name="file_path" accept=".pdf" required class="hidden"
                           onchange="document.getElementById('file-label').textContent = this.files[0]?.name || 'Klik untuk pilih file PDF'">
                </label>
            </div>
            ...
            <div class="flex items-center justify-end gap-3 pt-2">
                ...
                <button type="submit" class="px-5 py-2 text-sm font-semibold text-white bg-bmkg-blue rounded-xl hover:bg-bmkg-navy transition-colors shadow-sm">Simpan Dokumen</button>
            </div>
        </form>
    </div>
</div>
