...
<div class="max-w-7xl mx-auto px-4 py-10">
    ...
    {{-- UNIT OPERASIONAL                                       --}}
    {{-- ═══════════════════════════════════════════════════════ --}}
    @if(count($unitOperasional) > 0)
    <div class="mb-10">
        {{-- Group header --}}
        <div class="flex items-center gap-3 mb-6">
            <div class="w-1.5 h-8 bg-bmkg-blue rounded-full"></div>
            <div>
                <h2 class="text-lg font-black text-bmkg-navy uppercase tracking-wide">Unit Operasional</h2>
                <p class="text-xs text-gray-400">Pelaksana tugas pokok teknis operasional</p>
            </div>
        </div>
        ...
        <div class="space-y-8">
            ...
            <div>
                ...
                <div class="flex items-center gap-2 mb-4 pl-1">
                    ...
                    <span class="text-xs text-gray-400 font-medium">{{ count($sub['members']) }} orang</span>
                </div>
    ...
    {{-- UNIT TATA USAHA                                        --}}
    {{-- ═══════════════════════════════════════════════════════ --}}
    @if($unitTataUsaha->isNotEmpty())
    <div class="mb-10">
        <div class="flex items-center gap-3 mb-6">
            <div class="w-1.5 h-8 bg-bmkg-blue rounded-full"></div>
            <div>
                <h2 class="text-lg font-black text-bmkg-navy uppercase tracking-wide">Unit Tata Usaha</h2>
                <p class="text-xs text-gray-400">Pengelolaan administrasi & keuangan</p>
            </div>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            @foreach($unitTataUsaha as $i => $p)
            @php $initials = collect(explode(' ', $p['nama']))->filter(fn($w) => strlen($w) > 0)->map(fn($w) => strtoupper($w[0]))->take(2)->join(''); @endphp
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:ring-1 hover:ring-bmkg-blue/30 transition-all duration-300 overflow-hidden group">
                <div class="aspect-square relative overflow-hidden bg-gray-50">
                    @if(!empty($p['foto']))
    ...
    <div class="mb-10">
        <div class="flex items-center gap-3 mb-6">
            <div class="w-1.5 h-8 bg-bmkg-blue rounded-full"></div>
            <div>
                <h2 class="text-lg font-black text-bmkg-navy uppercase tracking-wide">PPNPN</h2>
                <p class="text-xs text-gray-400">Pegawai Pemerintah Non Pegawai Negeri</p>
            </div>
        </div>

        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4">
            @foreach($ppnpn as $i => $p)
            @php $initials = collect(explode(' ', $p['nama']))->filter(fn($w) => strlen($w) > 0)->map(fn($w) => strtoupper($w[0]))->take(2)->join(''); @endphp
            <div class="bg-white rounded-xl border border-gray-200 shadow-sm hover:shadow-md hover:ring-1 hover:ring-bmkg-blue/30 transition-all duration-300 overflow-hidden group">
                <div class="aspect-square relative overflow-hidden bg-gray-50">
                    @if(!empty($p['foto']))
    ...
    {{-- Empty state --}}
    @if(!$kepalaUpt && count($unitOperasional) === 0 && $unitTataUsaha->isEmpty() && $ppnpn->isEmpty())
